...
import { GroupObject, DeviceObject } from '../../../../@models';
...
export class RoomSelectorComponent implements OnInit, OnDestroy {
  ...
  public device:  {name:string; type:string; group:string;}
  ...
  ngOnInit() {
    ...
    this.selectRoom('hallway');
    this.select.emit('hallway');
  }
  ...
  addDevice(){
    this.dialogService.open(AddDeviceComponent)
      .onClose.subscribe((device:DeviceObject)=>{
        if(device){
          this.select.emit(this.getGroup(device.group).slug);
          this.selectedRoom = this.getGroup(device.group).slug;
          this.sortRooms();
        }
      });
  }

  getGroup(id:string):GroupObject{
    console.log(id, this.groups);
    
    return this.groups.filter((v:any)=>v._id == id)[0];
